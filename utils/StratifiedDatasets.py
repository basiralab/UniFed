...
class StratifiedData:
    # Constructor
    def __init__(self, data_flags: list, split_ratio=0.2, test_ratio=0.2, validation_ratio=0.1, split_method="stratify", amount=8000, num_clients=8):
    ...
    def get_ordered_client_split(self, client_data, client_label):
        splited_client_data = np.array_split(client_data, self.num_clients)
        splited_client_label = np.array_split(client_label, self.num_clients)
    ...
    def get_clients_data(self, client_splitting_method="ordered"):
        ...
        for (each_client_data, each_client_label) in zip(self.clients_data, self.clients_labels):
            ...
            # print("Yaaaay s")
            # print(each_client_label)

            if client_splitting_method == "ordered":
                splited_client_data, splited_client_label = self.get_ordered_client_split(each_client_data, each_client_label)
                # print(len(splited_client_data))
